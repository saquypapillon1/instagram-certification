...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function validateStep1() {
        ...
        if (!phone) {
            showError(document.getElementById("instagramPhone"), "Le numéro de téléphone est requis");
            isValid = false;
        } else if (!validatePhone(phone)) {
            showError(document.getElementById("instagramPhone"), "Format de téléphone invalide");
            isValid = false;
        }
    ...
    if (fileInput) {
        fileInput.addEventListener("change", function(e) {
            ...
            if (fileName) {
                ...
                fileSelected.textContent = `Fichier sélectionné : ${fileName}`;
...
Set-Content -Path "scripts\form.js" -Value $jsContent -Encoding UTF8